...
- 同步 148—150 Cordis / DeepSeek Harness 三篇到公开博客，分别设置 `publishAt` 为 2026-08-27 19:00、2026-08-30 19:00、2026-09-02 00:00；公众号链接待正式发出后回填。封面与正文解释图已从 `writing-craft` 转为 WebP 后入库，文章 URL 使用纯语义 slug。

### 维护

- 更新定时发布维护口径：未来文章详情页和 OG 图提前生成，列表入口按 `data-publish-at` 运行时隐藏，到点刷新即可见；RSS 仍按构建时刻过滤。构建产物检查新增未来文章详情页 / OG 图断言。
- 跟进文章 URL 去期号规则：077、148、149、150 改为纯语义文章路径，并为旧期号路径保留重定向。
- 回填 077 期公众号正式链接。
...
- 同步第 077 期《医疗大模型高分之后,为什么还不能直接用?》到公开博客：新增文章页 `077-medical-ai-benchmark`，`publishAt` 设为 `2026-08-22T12:53:28+08:00` 立即公开；封面从 `writing-craft` 转为 WebP 后入库，公众号链接待正式发出后回填。

### 维护

- 首页最新文章卡、首页已发布主卡、文章页封面和归档页缩略图统一改为完整展示封面图，不再用 `object-fit: cover` 裁切；构建产物检查同步断言首页两处主封面使用 `contain`，避免公众号封面文字被切掉。

## 2026-08-11

### 新增

- 同步公众号第 100 期《开源模型不再追赶，它开始讨价还价了》到公开博客：新增文章页 `100-open-models-negotiation`，补入公众号原文链接和 2026-08-10 21:07 实际发布时间；公开封面图与正文图从 `writing-craft` 转为 WebP 后入库。

## 2026-08-06

### 维护

- 补齐过往公众号旧文的博客配图：从公开公众号链接重新抓取 2022《重拾写作 1——砍柴、放羊与元放羊》和 2023《ChatGPT：都什么年代，谁在搞传统 NLP？》原文图片，入库到 `public/images/archive/2022-chopping-herding-meta-herding/` 与 `public/images/archive/2023-chatgpt-traditional-nlp/`；PNG/JPEG 转 WebP，GIF 保留原格式。两篇正文中 12 处「原公众号此处有图片」占位已替换为真实图片引用，`TODO.md` 移除旧文图片待补项，`npm test` 已通过。
- 修复 OG 图模板右侧品牌蓝弧被画布边缘裁切的问题：`src/pages/og/[slug].png.ts` 的蓝弧组从 `translate(830 320) scale(2)` 收回到 `translate(810 320) scale(2)`，理论右缘由 1218px 改为 1198px，完整落入 1200px 画布；构建产物检查新增模板边界断言，防止再次越界。
- 清理构建产物检查里的当前文章数据绑定断言：`scripts/check-public-output.mjs` 不再写死归档年份 `2021 / 2026` 或某一篇正文图的具体宽度值，改为检查归档页存在四位年份分组，以及所有懒加载正文图片都带 `width` / `height`。这样后续新增、删除或替换文章图片时不必同步改脚本里的旧数据常量；`npm test` 已通过。
- 清理 `TODO.md` 中已过期的第 004 / 005 期发布触发项和 001 / 003 / 004 / 005 公众号链接待补项：当前线上文章页、首页与 sitemap 已包含 004 / 005，且 001—005 的 `wechat`、日期与 `src/data/posts.ts` 双源字段已补齐；仅保留需要用户登录的 Search Console / Bing 站外提交事项。
- 第 003 期补 `wechat` 链接（公众号实测发送 2026-08-01 09:00）。**本仓的 `date: "2026-08-01"` 一直是对的**，反而是 `writing-craft` 记成 07-31，该仓已按实测值改名与更正——与 001 的情况相反（001 是本仓 07-30、对方 07-18，两边都错）。
- 补齐 / 更正全部 8 篇文章的公众号原文链接（`wechat` 字段），链接由本人从公众号后台提供、并经页面 `og:title` 与 `createTime` 逐条比对确认：001、004、005 为新增，002 由二维码短链 `weixin.qq.com/r/mp/...` 换为标准文章链接，2021《从元开始》由带 `__biz` 的长参数链接换为短链，2022《重拾写作 1》与 2023《ChatGPT：都什么年代，谁在搞传统 NLP？》为新增。003 的公众号链接随后已补齐，当前 001—005 均有标准公众号文章链接。
- **第 001 期 `date` 由 `2026-07-30` 更正为 `2026-07-24`**：公众号页面实测发送时间为 2026-07-24 20:42。此前本仓记 07-30、`writing-craft` 记 07-18（实为「与 002 合发准备」的计划日），三处互不一致；经用户裁决统一以平台实测时间为准，`writing-craft` 侧文件名与 `published_at` 已同步更正。**注意**：第 003 期本仓记 `2026-08-01` 是正确口径，`writing-craft` 已在 2026-08-06 按公众号实测值同步更正。
- **第 005 期 `publishAt` 由 `2026-08-06T21:00:00+08:00` 改为 `T08:00:00+08:00`**：公众号当日 08:00 实际发出，早于原定 21:00。改后该时间已过，构建即可产出文章页，本仓不再需要等到 21:00 才触发构建。
- **修复 `src/data/posts.ts` 与 Markdown frontmatter 的双源不同步**：本仓有两处发布元数据——`src/content/articles/*.md` 的 frontmatter 决定文章页正文，`src/data/posts.ts` 决定首页 / 列表 / 分类 / RSS / JSON-LD 结构化数据。只改前者时，005 的文章页能生成但**缺失全部 JSON-LD**（`npm test` 报 `article 005-agent-work-organization is missing "name":"ZENINEXU"`），因为 `posts.ts` 里 005 的 `publishAt` 仍是 21:00 而被过滤。该缺陷此前一直被定时过滤掩盖（005 页面根本不生成，断言循环遍历不到它），本次 005 提前上线才暴露。已同步 `posts.ts` 的 005 `publishAt`、001 `date` 与全部 7 条 `wechat`；`npm test` 全过（含构建产物断言）。**后续维护提醒：改发布时间或链接时两处都要改。**

## 2026-08-04

### 新增

- 预置第 004 期《AI 时代，你真正要养的羊是什么？》，同步公开正文、封面主视觉和“五层资产地图”正文解释图，`publishAt` 设为 `2026-08-04T21:00:00+08:00`，用于今晚 21:00 发布窗口；公众号平台链接待补。
- 预置第 005 期《Agent 不是聊天机器人，而是一种工作组织方式》，同步公开正文和 WebP 封面，`publishAt` 设为 `2026-08-06T21:00:00+08:00`，用于周四晚 21:00 发布窗口。
...
- 将发布过滤从仅按日期升级为优先按 `publishAt` 精确发布时间过滤；未写 `publishAt` 的旧文仍按北京时间当天 00:00 露出。
- 继续不使用 GitHub Actions cron 定时唤醒；未来文章只在构建时按当前时间过滤，发布时间到达后需由手动触发、推送触发或其它外部发布流程触发构建。
- 首页「已发布」区左侧主文章卡复用最新文章封面，填充与右侧列表等高产生的留白；构建产物检查同步断言封面图输出宽高属性。
- 首页「已发布」区左侧主文章卡改为展示次新文章，避免与首页 Hero 最新文章重复；构建产物检查同步断言两个入口 href 不相同。
- 文章页正文结束后新增轻量分享区，提供复制链接、系统分享回退复制和公众号入口；构建产物检查同步断言文章页分享控件存在。
- 新增轻量定时发布机制：构建期按当前时间只发布到点文章，未来文章不会生成页面、OG 图、首页入口、RSS、Pagefind 或 sitemap；workflow 保留推送触发和手动触发入口。
